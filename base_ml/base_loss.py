...
class XentropyLoss(_Loss):
    ...
    Args:
        reduction (str): 'mean' or None (sum). Defaults to 'mean'.
        class_weights (Optional[List[float]]): A list/tuple of length C (num_classes) specifying the relative weight for each class. If None, all classes get weight=1.
    """

    def __init__(self, reduction: str = "mean", class_weights: Optional[List[float]] = None) -> None:
        super().__init__(size_average=None, reduce=None, reduction=reduction)

        if class_weights is not None:
            # Convert to a tensor so we can apply easy broadcasting
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Check for non-finite values (NaN or Inf) in input or target
        if not torch.isfinite(input).all():
            print("Input contains non-finite values for xenropy loss")
        if not torch.isfinite(target).all():
            print("Target contains non-finite values for xenropy loss")
        ...
        #pred = input / torch.sum(input, -1, keepdim=True)   # Felicie: not needed as input is already softmaxed before (so this line just divides by 1)
        
        # manual computation of crossentropy :
        
        #pred = torch.clamp(pred, epsilon, 1.0 - epsilon)
        pred = torch.clamp(input, epsilon, 1.0 - epsilon)  # Clamp to avoid log(0)
        log_pred = torch.log(pred)

        # If user provided class weights, apply them
        # class_weights shape: (C,)
        # we want to broadcast to (1, 1, 1, C) so it multiplies each class's log_prob
        if self.class_weights is not None:
            w = self.class_weights.to(input.device).view(1, 1, 1, -1)
            weighted_log_pred = w * log_pred
        else:
            weighted_log_pred = log_pred

        # Cross Entropy for each pixel = - sum_c [ target_c * (log_pred_c * weight_c) ]
        loss = -torch.sum(target * weighted_log_pred, dim=-1, keepdim=True)  # shape: (N, H, W)
...
class DiceLoss(_Loss):
    ...
    Args:
        ...
        class_weights (Optional[List[float]]): If provided, each class c gets multiplied by w_c.
    """

    def __init__(self, smooth: float = 1e-3, class_weights: Optional[List[float]] = None) -> None:
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Check for non-finite values (NaN or Inf) in input or target
        if not torch.isfinite(input).all():
            print("Input contains non-finite values for dice loss")
        if not torch.isfinite(target).all():
            print("Target contains non-finite values for dice loss")

        input = input.permute(0, 2, 3, 1)
        target = target.permute(0, 2, 3, 1)

        # intersection over each class c
        # Summation over N, H, W => shape (C,)
        inse = torch.sum(input * target, dim=(0, 1, 2))
        l = torch.sum(input, dim=(0, 1, 2))
        r = torch.sum(target, dim=(0, 1, 2))

        dice_per_class = (2.0 * inse + self.smooth) / (l + r + self.smooth)
        # Dice loss for each class = 1 - dice_score
        loss_per_class = 1.0 - dice_per_class

        # Apply class weights if available
        if self.class_weights is not None:
            w = self.class_weights.to(input.device)
            # Weighted loss per class
            loss_per_class = loss_per_class * w

        # Sumacross classes
        loss = torch.sum(loss_per_class)
...
class MSELossMaps(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Check for non-finite values (NaN or Inf) in input or target
        if not torch.isfinite(input).all():
            print("Input contains non-finite values for mse loss")
        if not torch.isfinite(target).all():
            print("Target contains non-finite values for mse loss")
...
class MSGELossMaps(_Loss):
    ...
    def forward(
        ...
        # Check for non-finite values (NaN or Inf) in input or target
        if not torch.isfinite(input).all():
            print("Input contains non-finite values for msge loss")
        if not torch.isfinite(target).all():
            print("Target contains non-finite values for msge loss")
...
class FocalTverskyLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input (torch.Tensor): Predictions, logits (without Softmax (Félicie: non avec ?)). Shape: (B, C, H, W)
        ...
        # Check for non-finite values (NaN or Inf) in input or target
        if not torch.isfinite(input).all():
            print("Input contains non-finite values for focal tversky loss")
        if not torch.isfinite(target).all():
            print("Target contains non-finite values for focal tversky loss")
        ...
        #input = torch.softmax(input, dim=-1).view(-1)  -> softmax mis par les auteurs mais par erreur ? Choix de l'enlever car contrairement à ce qui est indiqué, l'input a déjà reçu la softmax avant
...
class MCFocalTverskyLoss(FocalTverskyLoss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input (torch.Tensor): Predictions, logits (without Softmax (Félicie: non avec ?)). Shape: (B, num_classes, H, W)
        ...
        # Check for non-finite values (NaN or Inf) in input or target
        if not torch.isfinite(input).all():
            print("Input contains non-finite values for MC focal tversky loss")
        if not torch.isfinite(target).all():
            print("Target contains non-finite values for MC focal tversky loss")
        ...
        # Softmax
        #input = torch.softmax(input, dim=-1)  -> softmax mis par les auteurs mais par erreur ? Choix de l'enlever car contrairement à ce qui est indiqué, l'input a déjà reçu la softmax avant
