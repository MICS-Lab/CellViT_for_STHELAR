...
from cell_segmentation.utils.metrics import get_fast_pq, remap_label, cell_detection_scores, cell_type_detection_scores
...
class CellViTTrainer(BaseTrainer):
    ...
    Args:
        ...
        loss_fn_dict (dict): Dictionary with loss functions for each branch with a dictionary of loss functions.
            Name of branch as top-level key, followed by a dictionary with loss name, loss fn and weighting factor and state for the weight.
            Example:
            {
                "nuclei_binary_map": {"bce": {loss_fn(Callable), weight_factor(float), state(str)}, "dice": {loss_fn(Callable), weight_factor(float), state(str)}},
                "hv_map": {"bce": {loss_fn(Callable), weight_factor(float), state(str), state(str), state(str)}, "dice": {loss_fn(Callable), weight_factor(float), state(str)}},
                "nuclei_type_map": {"bce": {loss_fn(Callable), weight_factor(float), state(str)}, "dice": {loss_fn(Callable), weight_factor(float), state(str)}}
                "tissue_types": {"ce": {loss_fn(Callable), weight_factor(float), state(str)}}
            }
            Required Keys are:
                * nuclei_binary_map
                * hv_map
    ...
    def __init__(
        ...
        for branch, loss_fns in self.loss_fn_dict.items():
            for loss_name in loss_fns:
                ...
                self.list_branch_lossname.append(f"{branch}_{loss_name}")
        self.batch_avg_tissue_acc = AverageMeter("Batch_avg_tissue_ACC", ":4.f")

        # if dynamic loss weights are used, track the weights for each loss
        self.weight_tracker = {}
        for branch, loss_fns in self.loss_fn_dict.items():
            for loss_name in loss_fns:
                if loss_fns[loss_name]["state"] == "dynamic":
                    self.weight_tracker[f"{branch}_{loss_name}"] = AverageMeter(
                        f"{branch}_{loss_name}_weight", ":.4f"
                    )
                    self.logger.info(f"Dynamic loss weights for {branch}_{loss_name}")
                elif loss_fns[loss_name]["state"] == "static":
                    self.logger.info(f"Static loss weights for {branch}_{loss_name}")
                else:
                    raise ValueError("Loss state not defined or wrong")
    ...
    def train_epoch(
        ...
        for branch, loss_fns in self.loss_fn_dict.items():
            for loss_name in loss_fns:
                ...
                if loss_fns[loss_name]["state"] == "dynamic":
                    self.weight_tracker[f"{branch}_{loss_name}"].reset()
        ...
        for batch_idx, batch in train_loop:
            ...
            train_loop.set_postfix(
                {
                    "Loss_avg": np.round(self.loss_avg_tracker["Total_Loss"].avg, 3),
                    "Loss_step": np.round(self.loss_avg_tracker["Total_Loss"].val, 3),
                    # "Dice": np.round(np.nanmean(binary_dice_scores), 3),
                    # "Pred-Acc": np.round(self.batch_avg_tissue_acc.avg, 3),
                    "Loss_avg_branches": [{branch_loss: np.round(self.loss_avg_tracker[branch_loss].val, 3)} for branch_loss in self.list_branch_lossname]
                }
            )
        ...
        # Add also dynamic loss weights to the scalar metrics
        for branch, loss_fns in self.loss_fn_dict.items():
            for loss_name in loss_fns:
                if loss_fns[loss_name]["state"] == "dynamic":
                    scalar_metrics[f"{branch}_{loss_name}_weight/Train"] = self.weight_tracker[f"{branch}_{loss_name}"].avg

        self.logger.info(
            f"{'Training epoch stats:' : <25} "
            # Losses
            f"Loss: {self.loss_avg_tracker['Total_Loss'].avg:.4f} - "
            f"Losses: {', '.join([f'{branch_loss}: {np.round(self.loss_avg_tracker[branch_loss].avg, 4)}' for branch_loss in self.list_branch_lossname])} - "
            # Dynamic Loss Weights
            f"Dynamic Loss Weights: {', '.join([f'{branch_loss}: {np.round(self.weight_tracker[branch_loss].avg, 4)}' for branch_loss in self.list_branch_lossname if branch_loss in self.weight_tracker])} - "
            # Metrics
        ...
        if self.mixed_precision:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                if (
                    ...
                    # # ### CHOOSE CLIPPING OR NOT GRADIENTS + LOGGING OR NOT THE GRADIENTS ###
                    self.scaler.unscale_(self.optimizer)
                    # # torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)  # CHOOSE MAX NORM
                    global_step = epoch * num_batches + batch_idx
                    if global_step % 15000 == 5:
                      for name, param in self.model.named_parameters(): # Log gradients to TensorBoard
                          if param.grad is not None:
                            try:
                                self.writer.add_histogram(f"gradients/{name}", param.grad, global_step)
                            except:
                                print(f"Error in logging gradients for {name} / grad: {param.grad} / global_step: {global_step}")
                                #raise Exception(f"Error in logging gradients for {name} / grad: {param.grad} / global_step: {global_step}")
                    # # ### END CHOOSE ###
    ...
    def validation_epoch(
        ...
        # Adding also the precision, recall and f1 score for each cell type
        ...
        # Get global to add also the precision, recall and f1 score
        ...
        scalar_metrics = {
            ...
            "f1_detection/Validation": float(f1_d),
            "precision_detection/Validation": float(prec_d),
            "recall_detection/Validation": float(rec_d),
        }
        ...
        for branch, loss_fns in self.loss_fn_dict.items():
            for loss_name in loss_fns:
                if loss_fns[loss_name]["state"] == "dynamic":
                    scalar_metrics[f"{branch}_{loss_name}_weight/Validation"] = self.weight_tracker[f"{branch}_{loss_name}"].avg
        ...
        # add also the precision, recall and f1 score for each cell type (nuclei metrics)
        for nuc_name, nuc_type in self.nuclei_types.items():
            ...
            scalar_metrics[f"{nuc_name}-f1_detection/Validation"] = float(f1_cell)
            scalar_metrics[f"{nuc_name}-precision_detection/Validation"] = float(prec_cell)
            scalar_metrics[f"{nuc_name}-recall_detection/Validation"] = float(rec_cell)
    ...
    def unpack_predictions(self, predictions: dict) -> DataclassHVStorage:
        ...
        # Check that predictions["nuclei_binary_map"] or predictions["nucei_tye_map"] does not contain NaN or Inf values
        if torch.isinf(predictions["nuclei_binary_map"]).any():
            print("nuclei_binary_map contains non finite values")
        if torch.isinf(predictions["nuclei_type_map"]).any():
            print("nuclei_type_map contains non finite values")
        ...
        # ==> Use stabilized softmax instead of just softmax ? (see: https://medium.com/@harrietfiagbor/softmax-uncovered-balancing-precision-with-numerical-stability-in-deep-learning-b8876490d411)
        #predictions["nuclei_binary_map"] = F.softmax(predictions["nuclei_binary_map"] - predictions["nuclei_binary_map"].max(dim=1, keepdim=True)[0], dim=1)
        #predictions["nuclei_type_map"] = F.softmax(predictions["nuclei_type_map"] - predictions["nuclei_type_map"].max(dim=1, keepdim=True)[0], dim=1)
        (predictions["instance_map"], predictions["instance_types"],) = self.model.calculate_instance_map(predictions, self.magnification)  # shape: (batch_size, H, W)
    ...
    def unpack_masks(self, masks: dict, tissue_types: list) -> DataclassHVStorage:
        ...
        # If eval mode then get also instance_types for gt to calculate precision, recall and f1 score
    ...
    def calculate_loss(
        ...
        for branch, pred in predictions.items():
            ...
            for loss_name, loss_setting in branch_loss_fns.items():
                ...
                if state == 'dynamic' and self.model.training: # if dynamic weight then calculate weight only for train set and use this same value for validation
                    weight = 1.0 / (loss_value.detach() + 1e-8)
                    loss_setting["weight"] = weight
                    self.weight_tracker[f"{branch}_{loss_name}"].update(weight.detach().cpu().numpy())
    ...
    def calculate_step_metric_validation(self, predictions: dict, gt: dict) -> dict:
        ...
        Returns:
            dict: Dictionary with metrics. Keys:
                binary_dice_scores, binary_jaccard_scores, tissue_pred, tissue_gt, true_inst_type_all, pred_inst_type_all, paired_all, unpaired_true_all, unpaired_pred_all
        """
        predictions = predictions.get_dict()
        gt = gt.get_dict()

        # Tissue Types logits to probs and argmax to get class
        predictions["tissue_types_classes"] = F.softmax(
            predictions["tissue_types"], dim=-1
        )
        pred_tissue = (
